...
if __name__ == "__main__":
    ...
    experimentfolder_segmentation = os.path.join(experimentfolder_parent, "Experiment0001_highresSeg_run_again/result_segmentation/", region)
    experimentfolder_rawdata =  os.path.join(experimentfolder_parent, "Experiment0001")
    experimentfolder_result = experimentfolder_parent + "Experiment0001_highresSeg_run_again"
    ...
    for i_time in timepointlist:
        # =============================================================================
        # define filepaths and load images
        # =============================================================================
        ...
        savefolder = os.path.join(experimentfolder_result,"processed_segmentation", region, i_time)
        ...
        im_binary_cancer = im_cancer > set_threshold  # +0.1;
        morphology.closing(im_binary_cancer,out=im_binary_cancer)
        skio.imsave(os.path.join(savefolder, "im_binary" + i_time + ".tif"), np.uint16(im_binary_cancer))
        ...
        skio.imsave(os.path.join(savefolder, 'labels_xy-integrated_gradients-correct_noexpand.tif'), np.uint16(im_seg_substracted_removedSmall))
        ...
        skio.imsave(os.path.join(savefolder, 'labels_xy-integrated_gradients-RGB-correct_noexpand.tif'),np.uint8(labels_3D_substracted_color))
