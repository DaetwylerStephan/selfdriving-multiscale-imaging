...
static inline result_t set_profile_8MT30V_50_VSS42(device_t id)
{
  ...
  stage_settings.HorizontalLoadCapacity = 3;
  ...
  const int8_t motor_information_Manufacturer_temp[16] = {80, 104, 121, 116, 114, 111, 110, 0, 111, 110, 116, 114, 111, 108, 32, 80};
  memcpy(motor_information.Manufacturer, motor_information_Manufacturer_temp, sizeof(int8_t) * 16);
  const int8_t motor_information_PartNumber_temp[24] = {86, 83, 83, 52, 50, 46, 50, 48, 48, 46, 49, 46, 50, 45, 72, 86, 0, 50, 48, 48, 45, 88, 0, 0};
  ...
  motor_settings.NominalVoltage = 42;
  motor_settings.NominalCurrent = 1.2000000476837158;
  motor_settings.NominalSpeed = 0;
  motor_settings.NominalTorque = 0.11999999731779099;
  motor_settings.NominalPower = 0;
  motor_settings.WindingResistance = 1.7000000476837158;
  motor_settings.WindingInductance = 3;
  motor_settings.RotorInertia = 77;
  ...
  const int8_t encoder_information_Manufacturer_temp[16] = {0, 97, 120, 111, 110, 0, 84, 101, 99, 104, 46, 0, 0, 0, 0, 0};
  memcpy(encoder_information.Manufacturer, encoder_information_Manufacturer_temp, sizeof(int8_t) * 16);
  const int8_t encoder_information_PartNumber_temp[24] = {0, 110, 99, 111, 100, 101, 114, 32, 77, 82, 32, 84, 121, 112, 101, 32, 77, 76, 0, 0, 0, 0, 0, 0};
  ...
  const int8_t gear_information_Manufacturer_temp[16] = {0, 97, 120, 111, 110, 32, 109, 111, 116, 111, 114, 0, 43, 32, 83, 116};
  memcpy(gear_information.Manufacturer, gear_information_Manufacturer_temp, sizeof(int8_t) * 16);
  const int8_t gear_information_PartNumber_temp[24] = {0, 52, 52, 48, 50, 55, 0, 52, 58, 49, 0, 105, 110, 32, 116, 111, 32, 98, 97, 115, 101, 41, 0, 0};
